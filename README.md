...
- **NumPy 基础笔记**：整理了学习 NumPy 时的常用操作和要点。
- **QR 分解**：实现了 Gram-Schmidt（经典+修正版）和 Householder 两种 QR 分解，并验证了重构误差与正交性。
- **随机化 SVD**：基于的随机化算法，支持设置目标秩 `k`、过采样参数 `p` 和幂迭代次数 `q`。
- **数值实验**：系统研究了过采样数 `p`、幂迭代次数 `q` 对随机化 SVD 精度的影响，并与标准 SVD 进行了精度和运行时间的对比。
...
| `矩阵切片与分块.py` | 矩阵索引、2×2 分块、离对角块低秩性观察 |
...
**核心结论**：经典 Gram-Schmidt 在病态矩阵下数值不稳定，修正版有所改善，Householder 方法最稳定——这也是实际选择 Householder 的原因。

## 第三阶段：随机化 SVD

学习当前大规模矩阵低秩近似的主流方法，这是我最感兴趣的方向。
...
| `精确SVD与随机SVD的效率对比.py` | 实验3：计时对比实验 |
